...
class ServiceImpl final : public packet::PacketStreamer::Service {
 public:
  ::grpc::Status StreamPackets(::grpc::ServerContext *context,
    ...
    std::vector<unsigned char> bt_properties_vec(bt_properties.ByteSizeLong());
    if (!bt_properties.SerializeToArray(bt_properties_vec.data(),
                                        bt_properties_vec.size())) {
    ...
    auto result = netsim::device::AddChipCxx(
        ...
        manufacturer, product_name, bt_properties_vec);
    if (result->IsError()) {
      return ::grpc::Status(::grpc::StatusCode::INVALID_ARGUMENT,
